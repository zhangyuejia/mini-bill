<template>
  <div class="page-container">
    ...
    <div v-if="userStore.currentFamily">
      <div class="query-bar">
        <el-select v-model="query.memberId" placeholder="选择成员" clearable style="width:160px" @change="fetchData"><el-option label="家庭" value="0" /><el-option v-for="m in members" :key="m.userId" :label="m.userName || '用户#'+m.userId" :value="m.userId" /></el-select>
      ...
      <div class="table-container">
        <el-table :data="filteredItems" v-loading="loading" border stripe>
          <el-table-column label="归属" min-width="150"><template #default="{ row }">{{ getMemberName(row.memberId) }}</template></el-table-column>
    ...
    <el-dialog v-model="showDlg" :title="isEdit ? '编辑储蓄项' : '新增储蓄项'" width="400px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="归属"><el-select v-model="form.memberId" placeholder="选择家庭成员或家庭" style="width:100%" :loading="membersLoading"><el-option label="家庭" value="0" /><el-option v-for="m in members" :key="m.userId" :label="m.userName || '用户#'+m.userId" :value="m.userId" /></el-select></el-form-item>
...
const filteredItems = computed(() => {
  ...
  if (query.value.memberId !== '' && query.value.memberId != null) items = items.filter(i => String(i.memberId) === String(query.value.memberId))
...
function getMemberName(userId) { if (String(userId) === '0') return '家庭'; const m = members.value.find(m => m.userId === userId); return m ? (m.userName || `用户#${userId}`) : `用户#${userId}` }
